 ...
 */
function buildTagControls(mesId, prompt) {
    return $(`
 ...
 * AutoPic 이미지 판별 로직
 */
function looksLikeAutopicImage(title) {
...
async function attachTagControls(mesId) {
    ...
    $images.each(function() {
        ...
        // ── 이미 래퍼 안에 있는 경우 ──
        if ($img.parent().hasClass('autopic-tag-img-wrapper')) {
            // 래퍼는 있지만 컨트롤 버튼이 없으면 재추가
            if ($img.parent().find('.autopic-tag-controls').length === 0) {
                const title = $img.attr('title') || "";
                const $controls = buildTagControls(mesId, title);
                $img.after($controls);
            }
            return; // 이미 완전한 상태면 스킵
        }

        // ── 새 이미지: AutoPic 이미지인지 판별 ──
